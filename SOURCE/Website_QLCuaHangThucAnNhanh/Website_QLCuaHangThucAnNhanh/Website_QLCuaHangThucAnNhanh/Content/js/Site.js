function changeQuantity(maMonAn, delta) {
    var quantityInput = document.getElementById('quantity_' + maMonAn);
    ...
    if (newQuantity >= 1) {
        ...
        // Gửi yêu cầu tới server để cập nhật số lượng (AJAX hoặc một cách khác)
        // Ví dụ sử dụng URL action (cập nhật giỏ hàng trên server)
        window.location.href = '@Url.Action("UpdateCart", "Home", new { MaMonAn = "__maMonAn__", SoLuong = "__soLuong__" }).replace("__maMonAn__", maMonAn).replace("__soLuong__", newQuantity)';
    }
}

document.addEventListener('DOMContentLoaded', function () {
    const nav = document.querySelector('.side-nav');  // Thanh điều hướng bên trái
    const footer = document.querySelector('footer');  // Phần footer
    const header = document.querySelector('header');  // Phần header
    const navHeight = nav.offsetHeight;  // Chiều cao của thanh điều hướng
    const headerHeight = header.offsetHeight;  // Chiều cao của header
    const footerTop = footer.offsetTop;  // Vị trí của footer

    window.addEventListener('scroll', () => {
        const scrollTop = window.scrollY;  // Vị trí cuộn hiện tại
        const stopPositionFooter = footerTop - navHeight;
        const stopPositionHeader = navHeight - headerHeight - 70;// Vị trí dừng của thanh nav

        // Điều kiện cuộn trang
        if (scrollTop >= stopPositionFooter) {
            // Nếu cuộn xuống dưới phần footer thì nav dừng lại ở vị trí trước footer
            nav.style.position = 'absolute';
            nav.style.top = `${stopPositionFooter}px`;
        } else if (scrollTop >= headerHeight) {
            // Nếu cuộn xuống dưới header thì nav cố định ở phía trên
            nav.style.position = 'fixed';
            nav.style.top = '0';
        } else {
            // Nếu ở trên phần header thì nav không cố định
            nav.style.position = 'absolute';
            nav.style.top = `${stopPositionHeader}px`;
        }
    });

    // Smooth scrolling và xử lý link active khi cuộn
    ...
    // Smooth scrolling khi click vào các link nav
    navLinks.forEach(link => {
